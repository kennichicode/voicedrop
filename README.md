...
### v1.1.0（2026-03-31）
- **メモリ使用量を大幅削減**：デフォルトのWhisperモデルを `whisper-large-v3-turbo`（約1.5GB）から `whisper-small`（約300MB）に変更。常駐時のメモリ消費が約1/4〜1/5になります。
- モデルは環境変数 `VOICEDROP_MLX_MODEL` で引き続き変更可能です。
